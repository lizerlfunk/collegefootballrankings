...
# observations
N <- nrow(games_18_BTstan)    # number of games in 2018
player1 <- games_18_BTstan$player1
player0 <- games_18_BTstan$player0
#for (n in 1:N) {
...
y <- games_18_BTstan$y #which team wins, from data
...
individual_posterior <-  sampling(individual_model, data = mle_model_data, pars = NA, 
         chains = 4, iter = 4000, warmup = 1000, diagnostic_file = "BTbayes18diagnostics.csv", verbose = FALSE, 
...
sink(file = "BTbayes18.csv")
summary(individual_posterior, "alpha", probs = c(0.05,0.5,0.95))$summary
sink(file = NULL)

sink(file = "BTbayes18-2.csv")
summary(individual_posterior, pars = c("alpha[126]", "alpha[127]", "alpha[128]", "alpha[129]", "alpha[130]"), probs = c(0.05,0.5,0.95))$summary
sink(file = NULL)


#indiv_post_ss <- as.data.frame(individual_posterior)

sink(file = "BTbayesranking18.csv")
summary(individual_posterior, "ranking",
      probs = c(0.05, 0.5, 0.95),
      digits_summary = 2, include = TRUE)$summary
sink(file = NULL)

sink(file = "BTbayesranking18-2.csv")
summary(individual_posterior, pars = c("ranking[126]", "ranking[127]", "ranking[128]", "ranking[129]", "ranking[130]"),
        probs = c(0.05, 0.5, 0.95),
        digits_summary = 2, include = TRUE)$summary
sink(file = NULL)
